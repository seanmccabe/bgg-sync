...
from custom_components.bgg_sync.const import DOMAIN, SERVICE_TRACK_GAME, SERVICE_RECORD_PLAY, CONF_BGG_USERNAME
...
async def test_service_track_game(hass):
    ...
    with patch(
        ...
        with patch("homeassistant.config_entries.ConfigEntries.async_update_entry") as mock_update:
            ...
            assert args.kwargs["options"]["game_data"]["123"]["custom_image"] == "http://img.com"
...
async def test_service_record_play(hass):
    ...
    with patch(
        "homeassistant.config_entries.ConfigEntries.async_entries",
        return_value=[entry]
    ), patch("custom_components.bgg_sync.BggDataUpdateCoordinator.async_config_entry_first_refresh"), \
       patch("homeassistant.config_entries.ConfigEntries.async_forward_entry_setups"), \
