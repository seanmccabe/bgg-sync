...
from custom_components.bgg_sync.const import DOMAIN, CONF_GAME_DATA, CONF_IMPORT_COLLECTION
...
async def test_sensor_creation(hass, mock_coordinator):
    ...
    assert any(isinstance(e, BggCollectionCountSensor) and e.key == "owned_boardgames" for e in args)
...
async def test_count_sensor(hass, mock_coordinator):
    ...
    sensor = BggCollectionCountSensor(mock_coordinator, "owned_boardgames", "Games", "mdi:icon")
...
async def test_setup_with_options(hass, mock_coordinator):
    ...
    entry.options = {
        CONF_GAME_DATA: {
            "999": {"custom_image": "test.jpg"} # Game not in coordinator dict, checking resilience
        },
    ...
    explicit_sensor = next(e for e in args if isinstance(e, BggGameSensor) and e.game_id == 999)
    assert explicit_sensor.user_data["custom_image"] == "test.jpg"
    assert explicit_sensor.name == "BGG Game 999" # Default fallback as mock data doesn't have 999
